    ...
    while (1) {
        ...
        if (is_negative) {
            ...
            if (abs_value >= 10) {
                ...
                vTaskDelay(pdMS_TO_TICKS(6));  // 6ms显示时间，恢复亮度
        ...
        // 【修改点】
        // 移除了循环末尾多余的 vTaskDelay(pdMS_TO_TICKS(10)) 和 taskYIELD()。
        // 循环内部的 vTaskDelay 已经足够让 IDLE 任务运行并重置看门狗。
        // 移除这个延迟会缩短总循环周期，从而提高亮度。
    }
}
...
void app_main(void)
{
    ...
    // 2. 启动数码管显示任务
    // 优先级设置为1（最低优先级，IDLE任务优先级为0），确保不会阻塞系统
    // 显示任务会定期让出CPU，确保IDLE任务有机会运行并重置看门狗
    ...
    // 4. 主循环
    // button组件会在内部任务中处理按键事件，主循环只需处理其他逻辑
    static uint32_t loop_count = 0;
    while (1) {
        loop_count++;
        
        // BLE通知（按键回调中已设置g_counter_updated标志）
        ...
        // 减少日志输出
        if (loop_count % 200 == 0) {
            ESP_LOGI(TAG, "状态: 计数器=%ld", g_counter);
        }
